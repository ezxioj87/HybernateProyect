...
public class RepositorioPersona {
	...
	public static Persona consultarPersona(Integer idPersona) {
		...
		try {
			...
			Persona persona=(Persona)sesion.createQuery("from Usuario USU2 where USU2.idUsuario = :idPersona")
					.setParameter("idPersona", idPersona).uniqueResult();

			persona.getTelefonos().stream().forEach(telefono -> System.out.println(telefono.getNumTlf()));
